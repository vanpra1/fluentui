# Change Log - @fluentui/cra-template

This log was last generated on Mon, 20 Sep 2021 12:52:04 GMT and should not be manually modified.

<!-- Start content -->

## [8.0.6](https://github.com/microsoft/fluentui/tree/@fluentui/cra-template_v8.0.6)

Mon, 20 Sep 2021 12:52:04 GMT 
[Compare changes](https://github.com/microsoft/fluentui/compare/@fluentui/cra-template_v8.0.5..@fluentui/cra-template_v8.0.6)

### Patches

- Bump @fluentui/cra-template to v8.0.6 ([commit](https://github.com/microsoft/fluentui/commit/6aa2ac7896128bfb8300b9875be1fce4624624d8) by lingfangao@hotmail.com)

## [8.0.5](https://github.com/microsoft/fluentui/tree/@fluentui/cra-template_v8.0.5)

Mon, 20 Sep 2021 10:51:15 GMT 
[Compare changes](https://github.com/microsoft/fluentui/compare/@fluentui/cra-template_v8.0.4..@fluentui/cra-template_v8.0.5)

### Patches

- Bump @fluentui/cra-template to v8.0.5 ([commit](https://github.com/microsoft/fluentui/commit/6fd2dc65870636be9d315a119c5d8e5496cc165c) by lingfangao@hotmail.com)

## 8.0.4

Mon, 20 Sep 2021 10:03:23 GMT

### Patches

- Bump @fluentui/cra-template to v8.0.4 (lingfangao@hotmail.com)

## 8.0.3

Mon, 20 Sep 2021 09:52:34 GMT

### Patches

- Bump @fluentui/cra-template to v8.0.3 (lingfangao@hotmail.com)

## [8.0.2](https://github.com/microsoft/fluentui/tree/@fluentui/cra-template_v8.0.2)

Thu, 16 Sep 2021 07:38:39 GMT 
[Compare changes](https://github.com/microsoft/fluentui/compare/@fluentui/cra-template_v8.0.1..@fluentui/cra-template_v8.0.2)

### Patches

- Update package description to clarify that the template is for `@fluentui/react` ([PR #19815](https://github.com/microsoft/fluentui/pull/19815) by elcraig@microsoft.com)
- Bump @fluentui/cra-template to v8.0.2 ([PR #19815](https://github.com/microsoft/fluentui/pull/19815) by gcox@microsoft.com)

## [8.0.1](https://github.com/microsoft/fluentui/tree/@fluentui/cra-template_v8.0.1)

Wed, 30 Jun 2021 07:38:35 GMT 
[Compare changes](https://github.com/microsoft/fluentui/compare/@fluentui/cra-template_v0.2.1..@fluentui/cra-template_v8.0.1)

### Patches

- Bump @fluentui/react version in template to 8 ([PR #18689](https://github.com/microsoft/fluentui/pull/18689) by elcraig@microsoft.com)

## [0.2.1](https://github.com/microsoft/fluentui/tree/@fluentui/cra-template_v0.2.1)

Mon, 01 Feb 2021 12:23:48 GMT 
[Compare changes](https://github.com/microsoft/fluentui/compare/@fluentui/cra-template_v0.2.0..@fluentui/cra-template_v0.2.1)

### Patches

- add prerelease in disallowed change type for version packages and fix current versioning. ([PR #16696](https://github.com/microsoft/fluentui/pull/16696) by xgao@microsoft.com)

## [0.2.0](https://github.com/microsoft/fluentui/tree/@fluentui/cra-template_v0.2.0)

Wed, 20 Jan 2021 12:22:46 GMT

### Minor changes

- feat(cra-template): boot up CRA template ([PR #16458](https://github.com/microsoft/fluentui/pull/16458) by jakubkonka@microsoft.com)
